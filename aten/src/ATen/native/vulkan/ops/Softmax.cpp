...
Tensor softmax_internal(
  ...
  api::utils::uvec3 global_workgroup_extents = v_output.extents();
  ...
  if (log_softmax) {
    ...
    if (dim == 1) {
      ...
      global_workgroup_extents.data[2u] = v_input_sizes[Layout::Activation4D::batch];
      ...
      if (dim == 0) {
        global_workgroup_extents.data[2u] =
            safe_downcast<int32_t>(std::ceil(v_input_sizes[Layout::Activation4D::channels] / 4.0));
        input_dim_stride.data[2u] = safe_downcast<int32_t>(std::ceil(v_input_sizes[Layout::Activation4D::channels] / 4.0));
      } else if (dim == 2) {
        global_workgroup_extents.data[1u] = 1;
        input_dim_stride.data[1u] = 1;
      } else { // dim == 3
        global_workgroup_extents.data[0u] = 1;
