...
TEST_F(VulkanAPITest, gelu) {
  ...
  // check tanh gelu approximation
  out_cpu = at::gelu(in_cpu, "tanh");
  out_vulkan = at::gelu(in_vulkan, "tanh");
...
TEST_F(VulkanAPITest, gelu_) {
  ...
  // check tanh gelu approximation
  cpu = at::rand({17, 197, 302, 5}, at::device(at::kCPU).dtype(at::kFloat));
  vulkan = cpu.vulkan();
  at::gelu_(cpu, "tanh");
  at::gelu_(vulkan, "tanh");

  check = almostEqual(cpu, vulkan.cpu());
  if (!check) {
    showRtol(cpu, vulkan.cpu());
  }
