...
class GroupFusionPass:
    ...
    def __init__(
        ...
        prevent_match_across_mutations=False,
    ):
        """
        Args:
            ...
            prevent_match_across_mutations (bool, optional): Prevent matching across mutations.
        """
...
def layer_norm_replacement(graph: torch.fx.Graph, nodes_to_fuse: List[torch.fx.Node]):
    ...
    with graph.inserting_before(nodes_to_fuse[0]):
        ...
        log.info(f"Fused {len(nodes_to_fuse)} layer norms")
...
layer_norm_fusion_pass = GroupFusionPass(
    ...
    # TODO rename `split_cat_fx_passes` to `pre_grad_fx_passes`
    extra_check=lambda m: config_flag("split_cat_fx_passes")(m)
